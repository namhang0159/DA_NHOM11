...
let update = async (req, res, next) => {
  uploadImage(req, res, async (err) => {
    ...
    try {
      ...
      for (let file of files) {
        const fileName = file.filename || path.basename(file.path);
        const urlPath = "http://localhost:8080/static/images/" + fileName;
...
let getProductVariant = async (req, res, next) => {
  let { product_id } = req.body;
  if (product_id === undefined) {
    try {
      let result = await Product_Variant.findAll();
    ...
    try {
      let result = await Product_Variant.findOne({
